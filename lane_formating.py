...
def nms_endpoints(lane_ends, thr):
    ...
    points = [item[0] for item in lane_ends]    # each first int
    ...
    for group in groups:
        ...
        results.append([center, lanes, dis])    # to do
...
class CollectRNNLanes(Collect):
    ...
    def target(self, results):
        ...
        for pts in gt_points:
            id_class = 1
            pts = convert_list(pts, self.down_scale)    # /8
            pts = sorted(pts, key=cmp_to_key(lambda a, b: b[-1] - a[-1]))
            pts = clamp_line(
                pts, box=[0, 0, mask_w - 1, mask_h - 1], min_length=1)
            if pts is not None and len(pts) > 1:
                valid_gt.append([pts, id_class - 1])    #(x,y /8)()()
        ...
        for l in valid_gt:
            ...
            gt_hm_lane_ends.append([point, l[0]])   # index + int(27,16)

        gt_hm_lane_ends = nms_endpoints(gt_hm_lane_ends, 1.01)#(gt,points,0)
        ...
        if len(gt_hm_lane_ends) >= 2:
            endpoints = [p[0] for p in gt_hm_lane_ends]
            for j in range(len(endpoints)):
                dis = min_dis_one_point(endpoints, j) # end  min distance
                if dis < 1.5 * radius[j]:
                    radius[j] = int(max(dis / 1.5, 1) + 0.49999)    # change r if close
        ...
        for (end_point, lines, _), r in zip(gt_hm_lane_ends, radius):
            ...
            draw_umich_gaussian(gt_hm[0], pt_int, r)
            mask_points = select_mask_points(
                pt_int, r, (hm_h, hm_w), max_sample=self.max_mask_sample)# 4 
            for line in lines:
                pos = np.zeros((mask_h), np.float32)    # 40
                pos_mask = np.zeros((mask_h), np.float32)
                line_array = np.array(line) # 8 2
                y_min, y_max = int(np.min(line_array[:, 1])), int(
                    np.max(line_array[:, 1]))
                mask_points = select_mask_points(
                    pt_int, r, (hm_h, hm_w), max_sample=self.max_mask_sample)   # to do
                reg = np.zeros((1, mask_h, mask_w), np.float32) # 1 40 100
                reg_mask = np.zeros((1, mask_h, mask_w), np.float32)    # 1 40 100

                extended_line = extend_line(line) # 0 + extend
                line_array = np.array(line)
                y_min, y_max = np.min(line_array[:, 1]), np.max(
                    line_array[:, 1])
                # regression
                m = np.zeros((mask_h, mask_w), np.uint8)    # 40 100
                lane_range = np.zeros((1, mask_h), np.int64)    # 1 40
                line_array = np.array(line)
                polygon = np.array(extended_line)
                polygon_map = draw_label(
                    m, polygon, 1, 'line', width=self.line_width + 9) > 0
                ...
                gt_seed_masks.append({
                    'reg': reg, # 1 40 100
                    'reg_mask': reg_mask,   # 1 40 100
                    'points': mask_points,  # 4 to do
                    'row': pos, # 40
                    'row_mask': pos_mask,   # 40
                    'range': lane_range,    # 1 40
                    'label': 0,
                })
            gt_masks.append({
                'points': mask_points,
