...
class CondLaneLoss(torch.nn.Module):
    ...
    def __init__(self, weights, num_lane_cls):
        ...
        self.shape_weight = weights[
            'shape_weight'] if 'shape_weight' in weights else shape_weight
        self.sim_weight = weights[
            'sim_weight'] if 'sim_weight' in weights else sim_weight
...
class CondLaneRNNLoss(CondLaneLoss):
    ...
    def forward(self, output, meta, **kwargs):
        ...
        if self.hm_weight > 0:
            hm_loss += self.crit(hm, kwargs['gt_hm'])   # batch 1 20 50
        if self.kps_weight > 0:
            kps_loss += self.crit_kp(kps, kwargs['gt_reg'],
                                     kwargs['gt_reg_mask']) # 1 29 80 200
        if self.state_weight > 0:
            state_loss += self.crit_ce(states, kwargs['gt_states']) #29 2
        if self.row_weight > 0:
            mask_softmax = F.softmax(mask, dim=3)   # 1 29 80 200
            pos = compute_locations(
                mask_softmax.size(), device=mask_softmax.device)
            row_pos = torch.sum(pos * mask_softmax, dim=3)
            row_loss += self.crit_kp(row_pos, kwargs['gt_rows'],
                                     kwargs['gt_row_masks'])    # 1 29 80
        if self.range_weight > 0:
            range_loss = self.crit_ce(lane_range, kwargs['gt_ranges'])
...
@LOSSES.register_module
class CondLaneRNNshapeLoss(CondLaneLoss):
    """for curvelanes rnn"""

    def __init__(self, weights, num_lane_cls):
        super(CondLaneRNNshapeLoss, self).__init__(weights, num_lane_cls)
        state_weight = 1.0
        self.state_weight = weights[
            'state_weight'] if 'state_weight' in weights else state_weight
    ...
    def forward(self, output, meta, **kwargs):
        hm, kps, mask, lane_range, states = output[:5]
        hm_loss, kps_loss, row_loss, range_loss, state_loss, shape_loss, sim_loss= 0, 0, 0, 0, 0, 0, 0
        hm = torch.clamp(hm.sigmoid_(), min=1e-4, max=1 - 1e-4)
        # losses for lane, seg_loss, ins_loss, kp_loss
        if self.hm_weight > 0:
            hm_loss += self.crit(hm, kwargs['gt_hm'])   # batch 1 20 50
        ...
        if self.row_weight > 0:
            ...
            row_loss += self.crit_kp(row_pos, kwargs['gt_rows'],
                                     kwargs['gt_row_masks'])    # 1 29 80
        if self.range_weight > 0:
            range_loss = self.crit_ce(lane_range, kwargs['gt_ranges'])  # 29 2 80

        if self.sim_weight > 0:
            loss_list = []
            for i in range(row_pos.shape[2] - 1):
                loss_list.append(row_pos[:,:,i] - row_pos[:,:,i+1])
            sim_loss = torch.cat(loss_list)
            sim_loss = nn.functional.smooth_l1_loss(sim_loss,torch.zeros_like(sim_loss))

        if self.shape_weight > 0:
            diff_list = []
            for i in range(row_pos.shape[2] // 2):
                diff_list.append(row_pos[:,:,i]-row_pos[:,:,i+1])
            
            for i in range(len(diff_list)-1):
                shape_loss += self.l1((diff_list[i]),diff_list[i+1])
            shape_loss /= len(diff_list) - 1
        ...
        if self.row_weight > 0:
            losses['row_loss'] = self.row_weight * row_loss
        if self.shape_weight > 0:
            losses['shape_loss'] = self.shape_weight * shape_loss
        if self.sim_weight > 0:
            losses['sim_loss'] = self.sim_weight * sim_loss
        if self.state_weight > 0:
            losses['state_loss'] = self.state_weight * state_loss
        if self.range_weight > 0:
            losses['range_loss'] = self.range_weight * range_loss
